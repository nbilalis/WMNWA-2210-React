...
function ReactionsProvider({ children }: ReactionsProviderProps) {
  ...
  const addReaction = (reaction: Reaction) => {
    setReactions((prev) =>
      prev.some((item) => item.reaction === reaction)
        ? prev.map((item) => (item.reaction === reaction ? { reaction, count: item.count + 1 } : item))
  ...
  const value = useMemo(() => ({ reactions, addReaction, removeReaction }), [reactions]) as ReactionsContextValue;
